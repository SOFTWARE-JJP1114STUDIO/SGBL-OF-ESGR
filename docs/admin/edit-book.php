...
if(isset($_POST['update'])) {
    ...
    $tome = $_POST['tome']; // Add this line
    $bookid = intval($_GET['bookid']);
    
    $sql = "UPDATE tblbooks SET BookName=:bookname, Tome=:tome, CatId=:category, AuthorId=:author, 
            Serie=:serie, Quantity=:quantity, Emplacement=:emplacement 
            WHERE id=:bookid";
    
    $query = $dbh->prepare($sql);
    $query->bindParam(':bookname', $bookname, PDO::PARAM_STR);
    $query->bindParam(':tome', $tome, PDO::PARAM_STR); // Add this line
...
<head>
    ...
    <title>Système de gestion de bibliothèque en ligne | Editer le livre</title>
...
<?php 
$bookid=intval($_GET['bookid']);
$sql = "SELECT tblbooks.BookName,tblbooks.Tome,tblbooks.Emplacement,tblcategory.CategoryName,tblcategory.id as cid,tblauthors.AuthorName,tblauthors.id as athrid,tblbooks.Serie,tblbooks.Quantity,tblbooks.id as bookid from  tblbooks join tblcategory on tblcategory.id=tblbooks.CatId join tblauthors on tblauthors.id=tblbooks.AuthorId where tblbooks.id=:bookid";
$query = $dbh -> prepare($sql);
$query->bindParam(':bookid',$bookid,PDO::PARAM_STR);
$query->execute();
...
<input class="form-control" type="text" name="tome" value="<?php echo htmlentities($result->Tome);?>"  required="required" />
</div>

<div class="form-group">
<label>Nom de la série<span style="color:red;">*</span></label>
<input class="form-control" type="text" name="serie" value="<?php echo htmlentities($result->Serie);?>"  required="required" />
</div>
...
<input class="form-control" type="text" name="emplacement" value="<?php echo htmlentities($result->Emplacement);?>" required />
</div>

 <div class="form-group">
 <label>Quantité</label>
 <input class="form-control" type="text" name="quantity" value="<?php echo htmlentities($result->Quantity);?>"/>
 </div>
 <?php }} ?>
